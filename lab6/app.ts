...
import { isTask, isComment } from './validate'
...
const handleErrors = (fn: RequestHandler) => (req: Request, res: Response, next: NextFunction) => fn(req, res, next).catch(next)
...
router.post('/', handleErrors(async (req, res) => {
    ...
    res.json(await db.get(id.toHexString()))
}))
